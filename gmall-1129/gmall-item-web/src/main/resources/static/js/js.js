$(function(){
	    ...
	    function switchSkuId() {
			...
			for(i=0;i<valueDivs.length;i++){
                ...
                if(valueIdsStr.length>0){
                    valueIdsStr+="|";
				}
                valueIdsStr+=valueId;

			}
			console.log("valueIdsStr:"+valueIdsStr);

			var valueIdsSkuIdJson = $("#valueIdsSkuIdJson").val();
			var valueIdsSkuIdJsonObj = JSON.parse(valueIdsSkuIdJson);
            var skuIdTarget =valueIdsSkuIdJsonObj[valueIdsStr];
            console.log("skuIdTarget:"+skuIdTarget);
            if(skuIdTarget){
